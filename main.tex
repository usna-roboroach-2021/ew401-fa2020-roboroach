...
\par Figure \ref{fig:prototype} shows a basic schematic for our PCB concept. The backpack itself depicted as a teal oval on the cockroaches back will be roughly 25x48mm in size. It will be mounted onto the cockroaches back with double sided tape. On the right side of figure \ref{fig:prototype}, you see that the components of the circuit board will mainly be the electrode base that will be connected to the electrodes implanted in the cockroaches antennae, and the Tiny Pico which will take care of any coding and power supply needs we have. We opted to keep the design fairly simple to give the students more time to focus on making a more accurate control system for their own RoboRoach. 
\begin{figure}[ht!]
\centering
\includegraphics[scale=0.5]{prototype.JPG}
\caption{Basic Prototype for our PCB Backpack}
\label{fig:prototype}
\end{figure}
